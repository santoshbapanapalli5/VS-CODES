...
# -------------------------------------------------------------
# SESSION STATE MANAGEMENT (Fixes the deletion issue)
# -------------------------------------------------------------
# We save the default tasks into the session state so modifications/deletions stick permanently!
...
st.caption("💡 *Tip: To delete the 5th row (or any row), click its left-most checkbox/index cell and press **Delete/Backspace** on your keyboard.*")

# Interactive Data Editor hooked into session state
df_input = st.data_editor(
    st.session_state.tasks_df,
    num_rows="dynamic",
    use_container_width=True,
    key="tasks_editor", # Binding a key automatically updates session state on modifications
    column_config={
...
# Keep the main dataframe synced with the edited user data
...
if st.button("⚡ Generate Optimal Schedule & Analytics"):
    # Drop completely blank rows if the user added empty lines
    cleaned_tasks = df_input.dropna(subset=["Task Name"])
    
    if cleaned_tasks.empty:
        st.error("Please add at least one task to build a schedule!")
    else:
        # Preprocessing & Normalization
        tasks = cleaned_tasks.copy()
